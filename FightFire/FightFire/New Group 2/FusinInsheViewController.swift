...
let Brie = Jnaeoue[0]              //jsBridge
let amt = Jnaeoue[1]     //amount
let ren = Jnaeoue[2]      //currency
let OpWin = Jnaeoue[3]      //openWindow
...
internal class FusinInsheViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    

    var jsieiiae: Fuansbahs?
    var lsoien: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if jsieiiae!.ertaysd != nil {
            view.backgroundColor = UIColor.init(hexString: jsieiiae!.ertaysd!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = jsieiiae!.pueybsf!
        AppsFlyerLib.shared().appleAppID = jsieiiae!.fpiuwn!
...
//        
        ...
        Jnaeoue = jsieiiae!.kodune!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: jsieiiae!.dhoeun!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        lsoien = WKWebView(frame: .zero, configuration: cofg)
        lsoien!.allowsBackForwardNavigationGestures = true
        lsoien?.uiDelegate = self
        lsoien?.navigationDelegate = self
        view.addSubview(lsoien!)
        
        
        kaoieus = jsieiiae!.laoiuv!
        lsoien?.load(URLRequest(url:URL(string: kaoieus!)!))
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = jsieiiae!.zbasufs!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = jsieiiae!.zbasufs!.contains("I") ? view.safeAreaInsets.bottom : 0
            lsoien?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
